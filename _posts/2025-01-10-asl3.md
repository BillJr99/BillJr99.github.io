...
6. Test DVSwitch:
   ...
   Note that DVSwitch cannot parse the `#` private call character in a `tune` command using the `/opt/MMDVM_Bridge/dvswitch.sh tune 1234.brandmeister.network:12345!91` format (i.e., using `!9990#`).  Instead, run these as two commands:
   
   ```
   /opt/MMDVM_Bridge/dvswitch.sh tune 1234.brandmeister.network:12345
