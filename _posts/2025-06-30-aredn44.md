...
On the pi, create a file `/usr/local/bin/setup-mesh-routes.sh` with the same commands and addresses as above, in addition to a route for your tunnel endpoint `44.a.b.c` in case it is within one of the routing blocks like `44.0.0.0/9` (so that it doesn't go out over the same interface, but instead over your WAN `10.0.0.1` and `wlan1` (use your WAN gateway IP and interface here):
