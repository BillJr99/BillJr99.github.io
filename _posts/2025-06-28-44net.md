...
My hAp ac2 has two wireless radios on 2.4 and 5 ghz (`wlan1` and `wlan2`, respectively).  Optionally, you can configure your node as a WiFi client or hotspot to connect to the Internet or to create a hotspot.  If you configure a WiFi client, be sure it (i.e., `wlan1`) is acting as a DHCP client in addition to the `ether1` port.  

Here, we will configure `wlan1` as a client and put it on the bridge with our ethernet LAN ports, and then add `ether1` as a DHCP client acting as the WAN port.  I will also set `wlan2` to act as a WiFi hotspot so that my clients can connect to it.
...
Set `wlan1` to `ap-bridge` mode and associate it with a security profile with your wireless base station's pre-shared keys.  Then add it as a DHCP client and to the bridge:
...
`wlan2` can be used as an access point hotspot (`station` mode) for inbound connections from your clients.  I configured `wlan2` as an access point, and associated it with a security profile that included my pre-shared keys for the wireless network I was creating (similar to the WiFi client I established on `wlan1`).  To do this, I will remove `wlan2` from the bridge if it is already there, set `wlan2` to be a wireless `station`, and set `wlan2` as a DHCP client so that it hands out 44net IP addresses like its counterparts.
